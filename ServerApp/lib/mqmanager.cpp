...
void MQueueManager::openQueue(char *mqPath)
{
    ...
    if ((m_mqueue = mq_open(m_mqPath, O_RDWR | O_CREAT, 0660, &m_attr)) == -1)
    {
        perror("open queue");
        exit(EXIT_FAILURE);
    }
...
MMESSAGE MQueueManager::createMessage(const int id, const char *content)
{
    MMESSAGE msg;
    msg.id = id;
...
void MQueueManager::receiveMQueue()
{
    ...
    if (mq_receive(m_mqueue, m_message.content, m_attr.mq_msgsize, NULL) == -1)
    {
        perror("receiving message");
        exit(EXIT_FAILURE);
    }
    cout << "Message received:" << m_message.content << endl;
}
